...
import { adminFetchUsers, adminToggleUserStatus, adminExportSubscribers } from "@/lib/api-helpers";
...
export default function UserManagement() {
  ...
  const handleToggleStatus = async (authId, isActive) => {
    try {
      ...
      setUsers((prev) =>
        prev.map((u) => (u.authId === authId ? { ...u, isActive: !isActive } : u))
      );
    } catch (err) {
  ...
  const handleNewsletterExport = async () => {
    try {
      ...
      link.setAttribute("download", `vouchiqo_newsletter_subscribers_${new Date().toISOString().split("T")[0]}.csv`);
                  ...
                  >
                    ...
                    <TableCell className="p-4">
                      {u.role === "customer" ? (
                        ...
                        >
                          {u.emailNotifications !== false ? "Subscribed" : "Unsubscribed"}
                        </Badge>
                      ) : (
