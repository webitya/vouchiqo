...
export default function CreateCoupon() {
  ...
  const isProOrEnterprise = merchant?.plan === "pro" || merchant?.plan === "enterprise";
  ...
  const handleSubmit = async (e) => {
    ...
    let payload = {
      ...
      maxClaims: formData.usageLimit ? parseInt(formData.usageLimit, 10) : undefined,
    ...
    if (listingType === "coupon") {
      payload = {
        ...payload,
        code: formData.code.toUpperCase() || `SAVE-${Math.floor(1000 + Math.random() * 9000)}`,
      ...
      payload = {
        ...
        discountType: "fixed",
        discountValue: (parseFloat(formData.originalPrice) || 0) - (parseFloat(formData.salePrice) || 0),
        ...
        <div className="lg:col-span-7 space-y-6">
          ...
          <div className="bg-brand-bg border border-brand-border p-4 rounded-xl shadow-sm space-y-3">
            ...
            <div className="grid grid-cols-3 gap-2">
              {[
                { id: "coupon", label: "Promo Code", desc: "Monospace promo code" },
                { id: "deal", label: "Sale/Flat Offer", desc: "Discounted price list" },
                { id: "special", label: "Special/Gift", desc: "Loyalty, BOGO, or Freebie" },
              ].map((t) => (
                <button
                  key={t.id}
                  type="button"
                ...
                >
                  <span className="block text-xs font-bold text-brand-navy">{t.label}</span>
              ...
              <div className="text-center py-8 space-y-5">
                ...
                <div className="space-y-1.5">
                  ...
                  <p className="text-xs text-brand-subtext max-w-xs mx-auto leading-relaxed">
                    Your campaign has been submitted to the platform. It is now active and ready for customers.
                  </p>
                </div>
                <Link
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          <div className="space-y-1.5">
                            ...
                            >
                              ...
                              <SelectContent className="bg-brand-bg border border-brand-border">
                                <SelectItem value="percentage" className="text-xs font-semibold">
                                  Percentage OFF (%)
                                </SelectItem>
                        ...
                        <div className="space-y-1.5">
                          ...
                          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                            {[
                              ...
                              <label key={o.id} className="border border-brand-border p-2 rounded-lg flex items-center gap-2 cursor-pointer text-xs font-bold bg-brand-surface hover:bg-slate-100 select-none">
                                <input
                                  type="radio"
                                  name="specialOfferType"
                                  value={o.id}
                                  checked={formData.specialOfferType === o.id}
                                  onChange={(e) => setFormData({ ...formData, specialOfferType: e.target.value })}
                        ...
                        <div className="space-y-1.5">
                          ...
                          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                            {[
                              ...
                              <label key={rc.id} className="border border-brand-border p-2 rounded-lg flex items-center gap-2 cursor-pointer text-[10px] font-bold bg-brand-surface hover:bg-slate-100 select-none">
                                <input
                                  type="radio"
                                  name="redemptionMethod"
                                  value={rc.id}
                                  checked={formData.redemptionMethod === rc.id}
                                  onChange={(e) => setFormData({ ...formData, redemptionMethod: e.target.value })}
                          ...
                          {formData.image ? (
                            ...
                            <>
                              ...
                              <span className="text-[10px] text-brand-subtext font-semibold">
                                {uploadingImage ? "Uploading..." : "Click to select a file"}
                              </span>
                            </>
                          )}
                          <input
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-1.5">
                      ...
                      >
                        ...
                        <SelectContent className="bg-brand-bg border border-brand-border">
                          <SelectItem value="static" className="text-xs font-semibold">
                            Static Codes (No Webhook API)
                          </SelectItem>
                          <SelectItem value="api" className="text-xs font-semibold">
                    ...
                    <div className={`p-4 rounded-xl border flex justify-between items-center ${
                      ...
                      <div className="space-y-1">
                        <div className="flex items-center gap-1.5">
                          <span className="text-xs font-bold text-brand-navy">Pin to Homepage Featured Deals</span>
                          {!isProOrEnterprise && <Lock className="w-3.5 h-3.5 text-slate-400" />}
                        </div>
                        <p className="text-[10px] text-brand-subtext font-semibold">
                          Starter and Growth plans are locked out. Elevation requires Pro plan.
                        </p>
                      </div>
                      ...
                      <input
                        ...
                        className={`w-4 h-4 cursor-pointer ${
                          !isProOrEnterprise ? "cursor-not-allowed opacity-50" : ""
                        }`}
                      />
                    </div>
                    ...
                    <div className="flex gap-2.5 p-3.5 rounded-lg bg-brand-surface border border-brand-border text-xs text-brand-subtext leading-relaxed">
                      ...
                      <span>
                        By submitting this campaign, you authorize Vouchiqo to list it immediately. Expired campaigns can be revived manually.
                      </span>
                    </div>
                  </div>
                )}
              ...
              <div className="p-5 space-y-4 font-semibold text-brand-text text-xs">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-[10px] text-brand-subtext leading-snug font-medium line-clamp-2">
                    {formData.description || "Enter detailed instructions or coupon details."}
                  </p>
                </div>
                ...
                {listingType === "coupon" ? (
                  ...
                  <div className="flex justify-between items-center py-2 border-y border-slate-100">
                    ...
                    <span className="bg-brand-warning text-white text-[9px] font-bold py-1 px-3 rounded-full uppercase tracking-wider">
                      Save ₹{(parseFloat(formData.originalPrice) || 0) - (parseFloat(formData.salePrice) || 0)}
                    </span>
                  </div>
                ) : (
                  <div className="border border-dashed border-brand-blue/30 bg-blue-50/50 rounded-xl p-3 flex justify-between items-center">
                    <span className="font-bold text-brand-blue uppercase text-[10px]">
                      {formData.specialOfferType === "gift" ? "🎁 Free Gift" :
                       formData.specialOfferType === "bogo" ? "🏷️ Buy 1 Get 1" :
                       formData.specialOfferType === "points" ? "⭐ Loyalty Points" :
                       formData.specialOfferType === "referral" ? "👥 Referral Reward" : "📦 Special Bundle"}
                    </span>
