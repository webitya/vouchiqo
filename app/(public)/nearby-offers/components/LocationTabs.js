 ...
 */
export default function LocationTabs({ activeTab, setActiveTab, groupedCoupons }) {
  const tabs = [
    { key: "nearest", label: "Nearest", icon: Activity, count: groupedCoupons.nearest.length },
    { key: "city", label: "City", icon: Building2, count: groupedCoupons.city.length },
    { key: "state", label: "State", icon: Map, count: groupedCoupons.state.length },
    { key: "all", label: "All Local", icon: SlidersHorizontal, count: groupedCoupons.all.length },
  ];
