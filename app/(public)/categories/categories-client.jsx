...
const CATEGORY_DESCRIPTIONS = {
  ...
  "home-improvement": "Tiles, sanitary, granite, hardware, paints, electrical fittings",
...
function getCategoryDescription(slug) {
  return CATEGORY_DESCRIPTIONS[slug] || "Verified local store offers and discount codes";
}

export default function CategoriesClient({ categories, totalCategories, totalCoupons }) {
        ...
        <aside style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <div style={{ background: "#ffffff", borderRadius: 6, border: "1px solid #e5e7eb", boxShadow: "0 1px 3px rgba(0,0,0,0.05)", padding: "16px" }}>
            ...
            <p style={{ fontSize: 12, color: "#4b5563", lineHeight: 1.6, margin: 0, overflow: "hidden", display: "-webkit-box", WebkitLineClamp: showMoreAbout ? "unset" : 3, WebkitBoxOrient: "vertical" }}>
              If there&apos;s a deal out there, we&apos;ve already found it. Vouchiqo brings you verified offers across fashion, electronics, food, travel, beauty and more — all from real physical stores near you.
            </p>
            <button
              ...
              {POPULAR_CATEGORIES.map((cat) => {
                ...
                return (
                  <Link key={cat.slug} href={`/category/${cat.slug}`} style={{ textDecoration: "none" }}>
                    <div
                      style={{ border: "1px solid #e5e7eb", borderRadius: 6, background: "#ffffff", padding: "16px 10px 12px", display: "flex", flexDirection: "column", alignItems: "center", gap: 8, transition: "all 0.2s ease-in-out", boxShadow: "0 1px 2px rgba(0,0,0,0.02)", textAlign: "center" }}
                      className="brand-card-hover"
                    >
                      <div style={{ width: 52, height: 52, display: "flex", alignItems: "center", justifyContent: "center", background: bg, borderRadius: 8 }}>
                        <Icon style={{ width: 24, height: 24, color: color }} />
                      </div>
                      <div>
                        <p style={{ fontSize: 13, fontWeight: 700, color: "#000000", margin: "0 0 3px" }}>{cat.title}</p>
                        <p style={{ fontSize: 10, color: color, fontWeight: 600, margin: "0 0 4px" }}>
                          {(cat.coupons || 0) + (cat.offers || 0)} Offers
                        </p>
                        <p style={{ fontSize: 10, color: "#6b7280", margin: 0, lineHeight: 1.3, display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>
          ...
          <section style={{ background: "#ffffff", borderRadius: 6, border: "1px solid #e5e7eb", boxShadow: "0 1px 3px rgba(0,0,0,0.05)", padding: "16px 20px 20px" }}>
            ...
            {filteredCategories.length > 0 ? (
              ...
              >
                {filteredCategories.map((cat) => {
                  ...
                  return (
                    <Link key={cat.slug} href={`/category/${cat.slug}`} style={{ textDecoration: "none" }}>
                      <div
                        style={{ border: "1px solid #e5e7eb", borderRadius: 6, background: "#ffffff", padding: "14px 10px", display: "flex", flexDirection: "column", alignItems: "center", gap: 8, transition: "all 0.2s ease-in-out", boxShadow: "0 1px 2px rgba(0,0,0,0.02)", textAlign: "center" }}
                        className="brand-card-hover"
                      >
                        <div style={{ width: 44, height: 44, display: "flex", alignItems: "center", justifyContent: "center", background: bg, borderRadius: 6 }}>
                          <Icon style={{ width: 20, height: 20, color: color }} />
                        </div>
                        <div>
                          <p style={{ fontSize: 12, fontWeight: 700, color: "#000000", margin: "0 0 2px" }}>{cat.title}</p>
                          <p style={{ fontSize: 10, color: color, fontWeight: 600, margin: "0 0 4px" }}>
                            {cat.total > 0 ? `${cat.total} Offers` : "Offers Available"}
                          </p>
                          <p style={{ fontSize: 10, color: "#6b7280", margin: 0, lineHeight: 1.3, display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>
                            {getCategoryDescription(cat.slug)}
                          </p>
              ...
              <div style={{ textAlign: "center", padding: "48px 0", color: "#9ca3af" }}>
                <p style={{ fontSize: 13 }}>No categories found for &quot;{searchQuery}&quot;</p>
                <button
